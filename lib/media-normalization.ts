...
const MEDIA_ALIASES: Array<{ canonical: string; aliases: string[] }> = [
  ...
  { canonical: "애드부스트스크린", aliases: ["애드부스트", "애드부스트스크린", "애드부스트 스크린", "애드부스트스크린(dooh)", "애드부스터", "애드부스터스크린"] },
...
export function canonicalMedia(value: string) {
  const key = normalizeKey(value || "");
  if (!key) return "미확인";
  const hit = MEDIA_ALIASES.find((entry) => entry.aliases.some((alias) => {
    const aliasKey = normalizeKey(alias);
    return key === aliasKey || key.includes(aliasKey) || (key.length >= 4 && aliasKey.includes(key));
  }));
...
export function canonicalProduct(mediaInput: string, placementInput: string, sourceSheet = "") {
  ...
  if (media === "네이버") {
    if (key.includes("커뮤니케이션") || key.includes("커뮤ad") || key.includes("커뮤")) return "커뮤니케이션 애드";
  ...
  if (media === "카카오") {
    ...
    if (key.includes("키워드") || key.includes("검색광고") || key.includes("sa")) return "검색광고";
  }
  if (media === "당근") return "네이티브 피드 광고";
  if (media === "키즈노트") {
    if (key.includes("이벤트배너")) return "이벤트배너(서비스)";
    if (key.includes("동영상")) return "메인배너 · 동영상형";
...
export function inferMediaMentions(value: string) {
  const key = normalizeKey(value || "");
  if (!key) return [];
  return MEDIA_ALIASES
    .filter((entry) => entry.aliases.some((alias) => key.includes(normalizeKey(alias))))
